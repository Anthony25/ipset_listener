    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => {
                ...
                // allowed
                let nb_threads_arc = nb_threads_arc.clone();
                wait_until_free_slot(&nb_threads_arc);
                thread::spawn(move || {
                    decrement_threads_and_handle_client(
                        &nb_threads_arc, stream
                    )
                });
            },
            Err(_) => {
...
fn wait_until_free_slot(nb_threads_arc: &Arc<(Mutex<u32>, Condvar)>) {
...
fn decrement_threads_and_handle_client(
        nb_threads_arc: &Arc<(Mutex<u32>, Condvar)>, stream: TcpStream) {
    let &(ref lock, ref cvar) = &**nb_threads_arc;
    info!("New client…");
        ...
        Some(capt) => {
            ...
            match action {
                ipset_act @ "a" | ipset_act @ "d" => {
                    match add_or_delete_mac(ipset_act, arg) {
...
fn add_or_delete_mac(
        ipset_act: &str, ipset_arg: &str) -> Result<String, String> {
    let re_mac: Regex = Regex::new(RE_MAC_PATTERN).unwrap();

    let mac_addr = match re_mac.captures(ipset_arg) {
